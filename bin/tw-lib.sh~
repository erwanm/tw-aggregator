...
function cloneAsTWCSTiddler {
    ...
    echo "title: $newTitle" >"$targetTiddler"
     ...
     # copy fields except title and tags (+ excluded fields if any)
    command=" cat "
    for field in $excludeFields; do
	command="$command | grep -v '^$field: ' "
    done
    head -n $(( $firstBlankLineNo - 1 )) "$sourceTiddlerFile" | grep -v '^title: ' | grep -v '^tags: ' | eval "$command" >>"$targetTiddler"
